...
from utils.eval_tools import calculate_explained_variance, visualise_batch, evaluate_agent, get_episodic_metrics, \
...
def main(config):
    ...
    for iteration in range(1, int(config.max_steps + 1) // config.sb3.n_steps):
        ...
        for batch, start_obs, future_obs in rollout_buffer.convert_transitions_to_rollout():
            ...
            loss = pg_loss + config.sb3.ent_coef * entropy_loss + config.sb3.vf_coef * v_loss + config.sb3.alignment_coef * alignment_loss
