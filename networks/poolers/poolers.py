...
class TransformerPooler(nn.Module):
    def __init__(self, obs_length, nhead = 4, dim_feedforward = 128, num_layers = 2):
        super().__init__()
        _transformer_pooler = nn.TransformerEncoderLayer(obs_length, nhead = nhead, dim_feedforward = dim_feedforward, 
                                                         batch_first = True)
        self.transformer_pooler = nn.TransformerEncoder(_transformer_pooler, num_layers = num_layers)
        self.outp_token = nn.Parameter(torch.randn((1, obs_length)), requires_grad = True)
        self.skill_token = nn.Parameter(torch.randn((1, obs_length)), requires_grad = True)
