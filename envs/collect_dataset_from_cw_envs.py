...
@hydra.main(config_path="../configs/env", config_name="cw-notarget-N4C11S1S1-hard")
def main(config):
    chunks, config.obs_size = 100, 224
    env = [getattr(cw_envs, config.env)(config, seed=i) for i in range(num_proc)]
    num_colors = len(config.COLORS)
    file_name = "/media/denis/data/OCRL_causal/" + "{config.env}-" f"N4C{num_colors}S1S1-Tr{num_tr}-Val{num_val}_{config.obs_size}.hdf5"
